...
# Use skill root as base directory
...
generate_html_output() {
	...
	local use_diagram_filter=${3:-true}
	(
		cd output
		cp "../assets/github-markdown.css" ./media/github-markdown.css
		cp "../assets/before.html" ./media/before.html
		cp "../assets/after.html" ./media/after.html
		
		# Build pandoc command
		local pandoc_cmd="pandoc \"$INPUT_MD\" \
			--extract-media=./media \
			-f gfm \
			-t html5 \
			-s \
		...
		# Add diagram filter only if available and requested
		if [ "$use_diagram_filter" = "true" ] && [ -f "../assets/diagram-simple.lua" ]; then
			pandoc_cmd="pandoc \"$INPUT_MD\" \
				--lua-filter=../assets/diagram-simple.lua \
				--extract-media=./media \
				-f gfm \
				-t html5 \
				-s \
